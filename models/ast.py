...
class ASTModel(nn.Module):
    ...
    """
    def __init__(self, label_dim=527, fstride=10, tstride=10, input_fdim=128, input_tdim=1024, imagenet_pretrain=True, audioset_pretrain=False, model_size='base384', verbose=True, domain_label_dim=527, multitask_label_dim=527, multitask_domain_label_dim=527, mix_beta=None):
        ...
        if audioset_pretrain == False:
            ...
            self.multitask_mlp_head = nn.Sequential(nn.LayerNorm(self.original_embedding_dim), nn.Linear(self.original_embedding_dim, multitask_label_dim)) # added for multitask learning
            self.multitask_domain_mlp_head = nn.Sequential(nn.LayerNorm(self.original_embedding_dim), nn.Linear(self.original_embedding_dim, multitask_domain_label_dim)) # added for multitask learning
        ...
        # now load a model that is pretrained on both ImageNet and AudioSet
            ...
            audio_model = ASTModel(label_dim=527, fstride=10, tstride=10, input_fdim=128, input_tdim=1024, imagenet_pretrain=False, audioset_pretrain=False, model_size='base384', verbose=False, domain_label_dim=527, multitask_label_dim=527, multitask_domain_label_dim=527)
