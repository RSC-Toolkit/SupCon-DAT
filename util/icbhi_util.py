...
def get_annotations(args, data_folder):
    ...
    if args.class_split == 'lungsound' or args.class_split in ['lungsound_meta', 'meta']: # 4-class
        ...
        for f in filenames:
            ...
            annotation_dict[f] = ann
    elif args.class_split in ['multitask', 'multitask_domain']:
        tmp = pd.read_csv(os.path.join(args.data_folder, 'icbhi_dataset/patient_diagnosis.txt'), names=['Disease'], delimiter='\t')
...
def _get_diagnosis_smart_label(disease, n_cls):
    if n_cls == 3:
        # 0: healthy, 1: Lung Parenchymal Disease, 2: Airway Disease
        if disease in ['간질성폐질환', '흉수', '폐렴']:
            return 1
        elif disease in ['천식-안정', '천식-악화', '만성폐쇄성폐질환-안정', '만성폐쇄성폐질환-악화']:
            return 2
        else: 
            return 0        
...
def _get_diagnosis_label(disease, n_cls):
    if n_cls == 3:
        # 0: healthy, 1: Lung Parenchymal Disease, 2: Airway Disease
        if disease in ['Pneumonia', 'LRTI']:
            return 1
        elif disease in ['URTI', 'Asthma', 'Bronchiolitis', 'COPD', 'Viral wheezing', 'Acute bronchiolitis']:
            return 2
        else: 
            return 0        
...
def get_meta_infor(metadata, args, meta_infor_len):
    if meta_infor_len == 8: ## ICBHI
        age = int(metadata[0])
        sex = int(metadata[1])
        loc = int(metadata[-3])
        dev = int(metadata[-2])
    ...
    elif args.meta_mode == 'age_data_loc_sex':
        if age >= 19:
            if data == 0 and loc == 0 and sex == 0:
                meta_label = 0
            elif data == 0 and loc == 0 and sex == 1:
...
def get_meta_infor_snubh_smart(metadata, args):
    label = int(metadata[0])
    age = int(metadata[2])
    sex = int(metadata[1])
    dev = int(metadata[3])
    ...
    if args.meta_mode == 'none':
        ...
        if sex == 0 and dev == 0: #
            meta_label = 0
        elif sex == 0 and dev == 1: #
            meta_label = 1
        elif sex == 0 and dev == 2: #
            meta_label = 2
        elif sex == 0 and dev == 3: #
            meta_label = 3
        elif sex == 0 and dev == 4:
            meta_label = 4
        elif sex == 1 and dev == 0: #
            meta_label = 5
        elif sex == 1 and dev == 1: #
            meta_label = 6
        elif sex == 1 and dev == 2: #
            meta_label = 7
        elif sex == 1 and dev == 3: #
            meta_label = 8
        elif sex == 1 and dev == 4:
            meta_label = 9
        else:
            raise ValueError
...
def get_individual_cycles_torchaudio(args, recording_annotations, metadata, data_folder, filename, sample_rate, n_cls):
    ...
    for idx in recording_annotations.index:
        ...
        if args.class_split == 'lungsound':
            ...
            disease = row['Disease']            
            sample_data.append((audio_chunk, _get_diagnosis_label(disease, n_cls), metadata))
        
        elif args.class_split in ['multitask', 'multitask_domain']:
            crackles = row['Crackles']
            wheezes = row['Wheezes']
            lung_label = _get_lungsound_label(crackles, wheezes, args.lung_cls, args)
            
            disease = row['Disease']
            disease_label = _get_diagnosis_label(disease, args.disease_cls)
                
            sample_data.append((audio_chunk, _get_lungsound_label(crackles, wheezes, args.lung_cls, args), _get_diagnosis_label(disease, args.disease_cls), get_meta_infor(metadata, args, len(metadata))))
    
    padded_sample_data = []
    if args.multitask or args.multitask_domain:
        for data, lung_label, disease_label, m_label in sample_data:
            data = cut_pad_sample_torchaudio(data, args) # --> resample to [1, 128000] --> 8 seconds
            padded_sample_data.append((data, lung_label, disease_label, m_label))
...
def get_individual_cycles_torchaudio_snubh(args, recording_annotations, metadata, data_folder, filename, sample_rate, n_cls, disease):
    #self.file_to_metadata[fpath] = torch.tensor([label, 0 if gender == 'M' else 1, int(age), 4 if device == 'Jabes' else None, disease]) #label, sex, age, device, disease
    sample_data = []
    ...
    if args.class_split == 'wheeze':
        ...
        lung_label = int(metadata[0])
        disease_label = _get_diagnosis_label(disease, args.disease_cls)
    
    data = cut_pad_sample_torchaudio(data, args) # --> resample to [1, 128000] --> 8 seconds
        
    padded_sample_data = []
    if args.multitask or args.mtultitask_domain:
        padded_sample_data.append((data, lung_label, disease_label, get_meta_infor(metadata, args, len(metadata))))
    else:
        padded_sample_data.append((data, label, metadata))
    
    return padded_sample_data
    

def get_individual_cycles_torchaudio_snubh_ver2(args, recording_annotations, metadata, data_folder, filename, sample_rate, n_cls):
...
def get_individual_cycles_torchaudio_smart(args, recording_annotations, metadata, data_folder, filename, sample_rate, n_cls, disease):
    #self.file_to_metadata[fpath] = torch.tensor([label, 0 if gender == 'M' else 1, int(age), 4 if device == 'Jabes' else None, disease]) #label, sex, age, device, disease
    sample_data = []
    fpath = os.path.join(data_folder, filename)
    data, sr = torchaudio.load(fpath)
    ...
    if args.class_split == 'wheeze':
        ...
        label = _get_diagnosis_smart_label(disease, n_cls)
    elif args.class_split in ['multitask', 'multitask_domain']:
        lung_label = int(metadata[0])
        disease_label = _get_diagnosis_smart_label(disease, args.disease_cls)
