...
class CLAPICBHIDataset(Dataset):
    def __init__(self, train_flag, transform, args, print_flag=True, mean_std=False):
        ...
        if self.train_flag and os.path.isfile('./data/training_icbhi_class_nums.pt'):
            self.class_nums = torch.load('./data/training_icbhi_class_nums.pt')
        
        cache_path = './data/training_icbhi_clap.pt' if self.train_flag else './data/test_icbhi_clap.pt'
        ...
        if not os.path.isfile(cache_path):
            ...
            for idx, filename in enumerate(self.filenames):
                ...
                for samples in sample_data:
                    data1, data2, data3 = samples[0], samples[1], samples[2] # audio_image, label, meta
            ...
            if self.train_flag:
                torch.save(self.audio_images, './data/training_icbhi_clap.pt')
            else:
                torch.save(self.audio_images, './data/test_icbhi_clap.pt')
        else:
            if self.train_flag:
                self.audio_images = torch.load('./data/training_icbhi_clap.pt')
            else:
                self.audio_images = torch.load('./data/test_icbhi_clap.pt')
...
class CLAPICBHINonPediatricsDataset(Dataset):
    def __init__(self, train_flag, transform, args, print_flag=True, mean_std=False):
        data_folder = os.path.join(args.data_folder, 'icbhi_dataset/audio_test_data')
        folds_file = os.path.join(args.data_folder, 'icbhi_dataset/patient_list_foldwise.txt')
        official_folds_file = os.path.join(args.data_folder, 'icbhi_dataset/official_split.txt')
        ...
        cache_path = './data/test_icbhi_non_pediatric_clap.pt'
        if not os.path.isfile(cache_path):
            # ==========================================================================
            """ get ICBHI dataset meta information """
            # store stethoscope device information for each file or patient
            ...
            for idx, filename in enumerate(self.filenames):
                sample_data = get_individual_cycles_torchaudio(args, annotation_dict[filename], self.file_to_metadata[filename], data_folder, filename, self.sample_rate, args.n_cls)
                for samples in sample_data:
                    data1, data2, data3 = samples[0], samples[1], samples[2] # audio_image, label, mea
            ...
            torch.save(self.audio_images, './data/test_icbhi_non_pediatric_clap.pt')
        else:
            self.audio_images = torch.load('./data/test_icbhi_non_pediatric_clap.pt')
...
class CLAPICBHIPediatricsDataset(Dataset):
    def __init__(self, train_flag, transform, args, print_flag=True, mean_std=False):
        data_folder = os.path.join(args.data_folder, 'icbhi_dataset/audio_test_data')
        folds_file = os.path.join(args.data_folder, 'icbhi_dataset/patient_list_foldwise.txt')
        official_folds_file = os.path.join(args.data_folder, 'icbhi_dataset/official_split.txt')
        ...
        cache_path = './data/test_icbhi_pediatric_clap.pt'
        if not os.path.isfile(cache_path):
            # ==========================================================================
            """ get ICBHI dataset meta information """
            # store stethoscope device information for each file or patient
            ...
            for idx, filename in enumerate(self.filenames):
                ...
                for samples in sample_data:
                    ...
                    if self.file_to_metadata[filename][0] < 19:
                        if self.file_to_metadata[filename][0] > 0:
                            audio_image_array.append(data1.squeeze(0).numpy())
                            label_array.append(data2)
                            meta_array.append(data3)
            ...
            torch.save(self.audio_images, './data/test_icbhi_pediatric_clap.pt')
        else:
            self.audio_images = torch.load('./data/test_icbhi_pediatric_clap.pt')
