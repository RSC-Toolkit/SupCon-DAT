...
from .icbhi_util import get_annotations, generate_fbank, get_individual_cycles_torchaudio, cut_pad_sample_torchaudio, get_meta_infor
...
class ICBHIDataset(Dataset):
    def __init__(self, train_flag, transform, args, print_flag=True, mean_std=False):
        ...
        if args.multitask:
            if self.train_flag and os.path.isfile('./data/training_ichbi_mtl_lung_class_nums.pt') and os.path.isfile('./data/training_icbhi_mtl_disease_class_nums.pt'):
                self.lung_class_nums = torch.load('./data/training_icbhi_mtl_lung_class_nums.pt')
                self.disease_class_nums = torch.load('./data/training_icbhi_mtl_disease_class_nums.pt')
            cache_path = './data/training_icbhi_mtl.pt' if self.train_flag else './data/test_icbhi_mtl.pt'        
        elif args.multitask_domain:
            if self.train_flag and os.path.isfile('./data/training_ichbi_mtld_lung_class_nums.pt') and os.path.isfile('./data/training_icbhi_mtld_disease_class_nums.pt'):
                self.lung_class_nums = torch.load('./data/training_icbhi_lung_class_nums.pt')
                self.disease_class_nums = torch.load('./data/training_icbhi_disease_class_nums.pt')
            cache_path = './data/training_icbhi_mtld.pt' if self.train_flag else './data/test_icbhi_mtld.pt'        
        else:        
            if self.train_flag and os.path.isfile('./data/training_ichbi_class_nums.pt'):
                self.class_nums = torch.load('./data/training_icbhi_class_nums.pt')
            cache_path = './data/training_icbhi.pt' if self.train_flag else './data/test_icbhi.pt'
        '''
        elif args.dataset == 'both':
            if self.train_flag and os.path.isfile('./data/training_ichbi_class_nums_both.pt'):
                self.class_nums = torch.load('./data/training_icbhi_class_nums_both.pt')
            cache_path = './data/training_icbhi_both.pt' if self.train_flag else './data/test_icbhi_both.pt'
        '''
        if not os.path.isfile(cache_path):
            ...
            for f in filenames:
                ...
                #self.file_to_metadata[f] = torch.tensor(np.append(info, self.file_to_device[f.strip()], 0)) #age, sex, adult_BMI, child_weight, child_height, chest_location, device, data
            ...
            if args.multitask:
                ...
                if self.train_flag:
                    lung_class_nums_cache_path = './data/training_icbhi_mtl_lung_class_nums.pt'
                    disease_class_nums_cache_path = './data/training_icbhi_mtl_disease_class_nums.pt'
                ...
                # ==========================================================================
                if self.train_flag:
                    torch.save(self.audio_images, './data/training_icbhi_mtl.pt')
                else:
                    torch.save(self.audio_images, './data/test_icbhi_mtl.pt')
                ...
                if self.train_flag:
                    lung_class_nums_cache_path = './data/training_icbhi_mtld_lung_class_nums.pt'
                    disease_class_nums_cache_path = './data/training_icbhi_mtld_disease_class_nums.pt'
                    if not os.path.isfile(lung_class_nums_cache_path):
                        torch.save(self.lung_class_nums, lung_class_nums_cache_path)
                    if not os.path.isfile(disease_class_nums_cache_path):
                        torch.save(self.disease_class_nums, disease_class_nums_cache_path)
                ...
                # ==========================================================================
                if self.train_flag:
                    torch.save(self.audio_images, './data/training_icbhi_mtld.pt')
                else:
                    torch.save(self.audio_images, './data/test_icbhi_mtld.pt')
            ...
            ##
                ...
                # ==========================================================================
                if self.train_flag:
                    torch.save(self.audio_images, './data/training_icbhi.pt')
                else:
                    torch.save(self.audio_images, './data/test_icbhi.pt')
                '''
                elif args.dataset == 'both':
                    if self.train_flag:
                        torch.save(self.audio_images, './data/training_icbhi_both.pt')
                    else:
                        torch.save(self.audio_images, './data/test_icbhi_both.pt')
                '''
        else:
            
            if args.multitask:
                if self.train_flag:
                    self.audio_images = torch.load('./data/training_icbhi_mtl.pt')
                    self.lung_class_nums = torch.load('./data/training_icbhi_mtl_lung_class_nums.pt')
                    self.disease_class_nums = torch.load('./data/training_icbhi_mtl_disease_class_nums.pt')
                else:
                    self.audio_images = torch.load('./data/test_icbhi_mtl.pt')
            
            elif args.multitask_domain:
                if self.train_flag:
                    self.audio_images = torch.load('./data/training_icbhi_mtld.pt')
                    self.lung_class_nums = torch.load('./data/training_icbhi_mtld_lung_class_nums.pt')
                    self.disease_class_nums = torch.load('./data/training_icbhi_mtld_disease_class_nums.pt')
                else:
                    self.audio_images = torch.load('./data/test_icbhi_mtld.pt')
            
            else:
                if self.train_flag:
                    self.audio_images = torch.load('./data/training_icbhi.pt')
                    self.class_nums = torch.load('./data/training_icbhi_class_nums.pt')
                else:
                    self.audio_images = torch.load('./data/test_icbhi.pt')
...
class ICBHINonPediatricsDataset(Dataset):
    def __init__(self, train_flag, transform, args, print_flag=True, mean_std=False):
        data_folder = os.path.join(args.data_folder, 'icbhi_dataset/audio_test_data')
        folds_file = os.path.join(args.data_folder, 'icbhi_dataset/patient_list_foldwise.txt')
        official_folds_file = os.path.join(args.data_folder, 'icbhi_dataset/official_split.txt')
        ...
        if args.multitask:
            cache_path = './data/test_icbhi_non_pediatric_mtl.pt'        
        elif args.multitask_domain:
            cache_path = './data/test_icbhi_non_pediatric_mtld.pt'        
        else:        
            cache_path = './data/test_icbhi_non_pediatric.pt'
        ...
        if not os.path.isfile(cache_path):
            ...
            for idx, filename in enumerate(self.filenames):
                ...
                if self.file_to_metadata[filename][0] < 0:
                    age_check_list.append(filename)
                    age_check_samples.extend(cycles_with_labels)
                    self.cycle_list.extend(cycles_with_labels)
                
                elif self.file_to_metadata[filename][0] >= 19:
                    age_check_list.append(filename)
                    age_check_samples.extend(cycles_with_labels)
                    self.cycle_list.extend(cycles_with_labels)
                
            print('patient {} samples {}'.format(len(age_check_list), len(self.cycle_list)))
            ...
            for index in range(len(self.audio_data)): #for the training set, 4142
                ...
                for aug_idx in range(self.args.raw_augment+1):
                    if aug_idx > 0:
                        ...
                        audio_image.append(image)
                if args.multitask:
                    self.audio_images.append((audio_image, label, disease_label, meta_label))
                elif args.multitask_domain:
                    self.audio_images.append((audio_image, label, disease_label, domain_label))
                else:
                    self.audio_images.append((audio_image, label, meta_label))
            
            # ==========================================================================
            if args.multitask:
                torch.save(self.audio_images, './data/test_icbhi_non_pediatric_mtl.pt')
            elif args.multitask_domain:
                torch.save(self.audio_images, './data/test_icbhi_non_pediatric_mtld.pt')
            else:                                                        
                torch.save(self.audio_images, './data/test_icbhi_non_pediatric.pt')
        else:
            if args.multitask:
                self.audio_images = torch.load('./data/test_icbhi_non_pediatric_mtl.pt')                
            elif args.multitask_domain:
                self.audio_images = torch.load('./data/test_icbhi_non_pediatric_mtld.pt')
            else:
                self.audio_images = torch.load('./data/test_icbhi_non_pediatric.pt')
...
class ICBHIPediatricsDataset(Dataset):
    def __init__(self, train_flag, transform, args, print_flag=True, mean_std=False):
        data_folder = os.path.join(args.data_folder, 'icbhi_dataset/audio_test_data')
        folds_file = os.path.join(args.data_folder, 'icbhi_dataset/patient_list_foldwise.txt')
        official_folds_file = os.path.join(args.data_folder, 'icbhi_dataset/official_split.txt')
        ...
        if args.multitask:
            cache_path = './data/test_icbhi_pediatric_mtl.pt'        
        elif args.multitask_domain:
            cache_path = './data/test_icbhi_pediatric_mtld.pt'        
        else:        
            cache_path = './data/test_icbhi_pediatric.pt'
        ...
        if not os.path.isfile(cache_path):
            ...
            for idx, filename in enumerate(self.filenames):
                ...
                if int(self.file_to_metadata[filename][0]) < 19:
                    if self.file_to_metadata[filename][0] > 0:
                        age_check_list.append(filename)
                        #print('filename {} age {}'.format(filename, self.file_to_metadata[filename][0]))
                        age_check_samples.extend(cycles_with_labels)
                        self.cycle_list.extend(cycles_with_labels)
            print('patient {} samples {}'.format(len(age_check_list), len(self.cycle_list)))
            ...
            # ==========================================================================
            if args.multitask:
                torch.save(self.audio_images, './data/test_icbhi_pediatric_mtl.pt')
            elif args.multitask_domain:
                torch.save(self.audio_images, './data/test_icbhi_pediatric_mtld.pt')
            else:
                torch.save(self.audio_images, './data/test_icbhi_pediatric.pt')
        else:
            if args.multitask:
                self.audio_images = torch.load('./data/test_icbhi_pediatric_mtl.pt')
            elif args.multitask_domain:
                self.audio_images = torch.load('./data/test_icbhi_pediatric_mtld.pt')
            else:
                self.audio_images = torch.load('./data/test_icbhi_pediatric.pt')
