...
from .icbhi_util import get_annotations, generate_fbank, get_individual_cycles_torchaudio, get_individual_cycles_torchaudio_snubh, cut_pad_sample_torchaudio
...
class CLAPBothDataset(Dataset):
    def __init__(self, train_flag, transform, args, print_flag=True, mean_std=False):
        ...
        if self.train_flag and os.path.isfile('./data/training_both_class_nums.pt'):
            self.class_nums = torch.load('./data/training_both_class_nums.pt')
                                
        cache_path = './data/training_both_clap.pt'
        if not os.path.isfile(cache_path):
            ...
            for idx, filename in enumerate(self.filenames):
                ...
                for samples in sample_data:
                    data1, data2, data3 = samples[0], samples[1], samples[2]
            ...
            torch.save(self.audio_images, './data/training_both_clap.pt')
        else:
            self.audio_images = torch.load('./data/training_both_clap.pt')
    
    def __getitem__(self, index):
        audio_image, label, meta_label = self.audio_images[index][0], self.audio_images[index][1], self.audio_images[index][2]z
