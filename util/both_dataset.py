...
from .icbhi_util import get_annotations, generate_fbank, get_individual_cycles_torchaudio, get_individual_cycles_torchaudio_snubh, cut_pad_sample_torchaudio, get_meta_infor_snubh, get_meta_infor
...
class BothDataset(Dataset):
    def __init__(self, train_flag, transform, args, print_flag=True, mean_std=False):
        ...
        if self.train_flag and os.path.isfile('./data/training_both_class_num.pt'):
            self.class_nums = torch.load('./data/training_both_class_nums.pt')
        cache_path = './data/training_both.pt'
        if not os.path.isfile(cache_path):
            ...
            ### SNUBH part
            self.data_folder_snubh = os.path.join(args.data_folder, 'snubh_dataset')
            meta_files = os.path.join(self.data_folder_snubh, 'training.csv' if self.train_flag else 'test.csv')
            ...
            for fpath, label, gender, age, device in zip(data_path, data_label, meta_gender, meta_age, meta_device):
                loc_id = fpath.split('-')[-1]
                loc_id = loc_id.split('.')[0]
                #print('fpath {} loc_id {}'.format(fpath, loc_id))
                if loc_id == '1':
                    loc_num = 2
                elif loc_id == '2':
                    loc_num = 1
                    ...
                    loc_num = 6
                self.file_to_metadata[fpath] = torch.tensor([0 if label == 'Nonwheezing' else 1, 0 if gender == 'M' else 1, int(age), 4 if device == 'Jabes' else None, loc_num]) #label, sex, age, device, loc
            #print('loc2 {} loc1 {} loc4 {} loc6 {}'.format(len(check_loc_num2), len(check_loc_num1), len(check_loc_num5), len(check_loc_num6)))
            #exit()            
            for idx, filename in enumerate(data_path):
                sample_data = get_individual_cycles_torchaudio_snubh(args, filename, self.file_to_metadata[filename], self.data_folder_snubh, filename, self.sample_rate, args.n_cls)
            ...
            if self.train_flag:
                class_nums_cache_path = './data/training_both_class_nums.pt'
            ...
            torch.save(self.audio_images, './data/training_both.pt')
        else:
            self.audio_images = torch.load('./data/training_both.pt')
            self.class_nums = torch.load('./data/training_both_class_nums.pt')
